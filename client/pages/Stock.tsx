...
import type { StockMaterial, MaterialCategory, MeasurementUnit, Supplier } from "@shared/crm-types"

// Mock data for measurement units
const measurementUnits: MeasurementUnit[] = [
  { type: "area", name: "Square Meters", abbreviation: "m²", baseConversion: 1 },
  { type: "area", name: "Square Centimeters", abbreviation: "cm²", baseConversion: 0.0001 },
  { type: "length", name: "Meters", abbreviation: "m", baseConversion: 1 },
  { type: "length", name: "Centimeters", abbreviation: "cm", baseConversion: 0.01 },
  { type: "length", name: "Millimeters", abbreviation: "mm", baseConversion: 0.001 },
  ...
  { type: "volume", name: "Milliliters", abbreviation: "mL", baseConversion: 0.001 },
...
export default function Stock() {
  ...
  const lowStockCount = materials.filter(m => m.currentStock <= m.minStock && m.currentStock > 0).length
  ...
  const getStockStatus = (material: StockMaterial): { status: string; color: string } => {
    if (material.currentStock === 0) {
      ...
      return { status: "Well Stocked", color: "bg-green-100 text-green-800 border-green-200" }
    } else {
      return { status: "Normal", color: "bg-blue-100 text-blue-800 border-blue-200" }
    }
  }
              ...
              <CardContent className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="font-medium">{formatCurrency(material.currentStock * material.unitPrice)}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Supplier</p>
                    <p className="font-medium truncate">{material.supplier.name}</p>
                  </div>
                  <div>
